...
const AdminNotifications = ({ user }) => {
  ...
  const tabs = useMemo(() => {
    ...
    // Default tabs for admin (fallback if no notifications yet)
    const defaultTabs = [
      "All",
      "Flagged User",
      "Dispute Report",
      ...
      <div className='w-full px-6 py-8'>
            ...
            <div className='flex items-center gap-4'>
              ...
              <div>
                ...
                <p className='text-gray-300 text-sm'>
                  {unreadCount} unread notifications • System & moderation
                  alerts
                </p>
              </div>
            </div>
        ...
        <div className='flex flex-wrap gap-2 mb-6'>
          {tabs.map((tab) => {
            const count =
              tab === "All"
                ? notifications.filter((n) => (showArchived ? n.read : !n.read))
                    .length
                : notifications.filter(
                    (n) => n.type === tab && (showArchived ? n.read : !n.read)
                  ).length;
            ...
            return (
              ...
              >
                {tab}
                {count > 0 && <Badge className='ml-2 text-xs'>{count}</Badge>}
              </Button>
            );
          })}
        </div>
