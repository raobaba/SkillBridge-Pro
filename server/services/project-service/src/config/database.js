...
const testConnection = async (retries = 3, delay = 2000) => {
  for (let i = 0; i < retries; i++) {
    try {
      const client = await pool.connect();
      console.log("🚀 Database connected successfully for project-service!");
      client.release();
      return true;
    } catch (error) {
      console.error(
...
const runMigrations = async () => {
  try {
    ...
    await runMigrations("project-service", {
      migrationsFolder: "./src/db/migrations",
      backupFolder: "./src/db/backups"
    });
    
    console.log = originalConsoleLog; // Restore console.log
    console.log("✅ Database migrations completed successfully for project-service");
  } catch (error) {
